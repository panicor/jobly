...
describe("POST /users", function () {
  test("works for admin to create non-admin", async () => {
  ...
  test("works for admin to create admin", async () => {
...
describe("GET /users", function () {
  ...
  test("unauthorized error for non-admin users", async () => {
    let resp = await request(app)
      .get("/users")
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("GET /users/:username", function () {
  ...
  test("unauthorized error for non-same user", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
...
describe("PATCH /users/:username", () => {
  ...
  test("unauthorizaton error if not same user", async () => {
    let resp = await request(app)
      .patch(`/users/u1`)
      .send({
        firstName: "New",
...
describe("DELETE /users/:username", function () {
  ...
  test("unauthorized error if not same user", async () => {
    let resp = await request(app)
      .delete(`/users/u1`)
      .set("authorization", `Bearer ${u2Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("POST /users/:username/jobs/:id", () => {
  ...
  test("not found error if no username found", async () => {
    let resp = await request(app)
      .post(`/users/test/jobs/${testJobIds[1]}`)
      .set("authorization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(404);
  });
